...
function normalizeSide(side){
  ...
  if(!Number.isInteger(side.active)||side.active<0||side.active>=n) side.active=side.selected.length?side.selected[0]:null;
  const defaults={
    status:null,statusTurns:0,hp:0,maxHp:0,boosts:{attack:0,defense:0,spAttack:0,spDefense:0,speed:0},
    protected:false,volatile:{},choiceMove:null,itemLost:false,itemTriggered:false,slowStart:false
  };
  const fill=(key,fallback)=>{if(!Array.isArray(side[key])) side[key]=side.team.map(()=>fallback()); while(side[key].length<n) side[key].push(fallback()); if(side[key].length>n) side[key]=side[key].slice(0,n);};
  fill('status',()=>null); fill('statusTurns',()=>0); fill('hp',()=>0); fill('maxHp',()=>0); fill('boosts',()=>({attack:0,defense:0,spAttack:0,spDefense:0,speed:0}));
  fill('protected',()=>false); fill('volatile',()=>({})); fill('choiceMove',()=>null); fill('itemLost',()=>false); fill('itemTriggered',()=>false); fill('slowStart',()=>false);
  side.boosts=side.boosts.map(x=>({...{attack:0,defense:0,spAttack:0,spDefense:0,speed:0},...(x||{})}));
  side.volatile=side.volatile.map(x=>x||{});

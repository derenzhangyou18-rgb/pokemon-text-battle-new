...
function doMove(b,attSide,defSide,move){
  ...
  if(power<=0){
    if(targetSide && effects.includes("remove_item") && d.hp[di]>0 && chance() && itemId(d)){const oldItem=getItem(itemId(d));d.itemId=null;d.item=null;defSide.itemLost[di]=true;b.log.push(`${d.name}の持ち物「${oldItem?.name||""}」がなくなった！`);}

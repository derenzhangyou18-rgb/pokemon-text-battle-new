...
function normalizeSide(side){
  ...
  fill('protected',()=>false); fill('volatile',()=>({})); fill('choiceMove',()=>null); fill('itemLost',()=>false); fill('itemTriggered',()=>false);
...
function makeBattle(playerIds){
  ...
  const b={id:crypto.randomUUID(),turn:1,phase:"selection",over:false,result:null,weather:null,pendingEnemyMoveId:null,
...
function applyEntryEffects(b,side,i){
  ...
  for(const {source,data} of effects){
    ...
    if(target.hp[ti]>0 && STAT_NAMES[data.effectStat]) addBoost(b,target,ti,data.effectStat,amount,side);
...
function doMove(b,attSide,defSide,move){
  ...
  if((effects.includes("stat_boost")||effects.includes("stat_drop")) && chance()){
    ...
    if(key){const target=move.effectTarget==="opponent"?defSide:attSide;const ti=move.effectTarget==="opponent"?di:ai;addBoost(b,target,ti,key,amount,attSide);}
  }
  ...
  const acc=Number(move.accuracy??100);if(!noMiss && acc<100&&Math.random()*100>=acc){b.log.push(`自分の${a.name}の技「${move.name}」は外れた！`);return {switched:false};}
  ...
  const r=preview;
  if(r.mult===0){b.log.push(`自分の${a.name}の技「${move.name}」！しかし効果がない！`);return {switched:false};}
  ...
  b.log.push(`自分の${a.name}の技「${move.name}」！${typeNote}相手の${d.name}に${damage}ダメージ。${extra}`);
...
function chooseEnemyMove(b){
  ...
  return (valid.length?valid:usable)[Math.floor(Math.random()*(valid.length?valid.length:usable.length))] || {id:"tackle",name:"たいあたり",type:"ノーマル",power:40,category:"physical",priority:0,target:"opponent",accuracy:100};
}
...
function runTurn(b,playerMoveId){
  ...
  const em=(b.pendingEnemyMoveId && getMove(String(b.pendingEnemyMoveId))) || chooseEnemyMove(b) || {id:"tackle",name:"たいあたり",type:"ノーマル",power:40,category:"physical",priority:0,target:"opponent",accuracy:100};
  b.pendingEnemyMoveId=String(em.id);
  prepareTurn(b);
  ...
  if(!b.over && b.phase==="battle"){
    ...
    if(!b.over && b.phase==="battle") { const next=chooseEnemyMove(b); if(next) b.pendingEnemyMoveId=String(next.id); }
  }
}

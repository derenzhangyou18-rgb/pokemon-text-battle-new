...
function publicPokemon(side,i,reveal=true){
  ...
  return {name:reveal?p.name:"？？？",hp:reveal?side.hp[i]:null,maxHp:reveal?side.maxHp[i]:null,selected:reveal?side.selected.includes(i):false,active:i===side.active,
    fainted:reveal?side.hp[i]<=0:false,status:reveal?(st?STATUS_NAMES[st]:null):null,type:reveal?speciesTypes(p):[],item:reveal&&itemId(p)?getItem(itemId(p))?.name:null,
...
function doMove(b,attSide,defSide,move){
  ...
  if(power<=0){
    if(targetSide && effects.includes("remove_item") && d.hp[di]>0 && chance() && itemId(d)){const oldItem=getItem(itemId(d)); if(oldItem?.effect!=="mega_stone"){d.itemId=null;d.item=null;defSide.itemLost[di]=true;b.log.push(`${d.name}の持ち物「${oldItem?.name||""}」がなくなった！`);} else { b.log.push(`${d.name}のメガストーンはなくならなかった！`); }}
  ...
  if(defSide.hp[di]>0 && effects.includes("remove_item") && chance() && itemId(d)){const oldItem=getItem(itemId(d)); if(oldItem?.effect!=="mega_stone"){d.itemId=null;d.item=null;defSide.itemLost[di]=true;b.log.push(`${d.name}の持ち物「${oldItem?.name||""}」がなくなった！`);} else { b.log.push(`${d.name}のメガストーンはなくならなかった！`); }}

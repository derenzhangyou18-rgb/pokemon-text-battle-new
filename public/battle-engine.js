...
function runOnlineTurn(b,playerAction,enemyAction){
  normalizeBattleState(b);
  if(b.over) return b;
  if(b.phase!=="battle") throw new Error("オンライン対戦の状態が不正です。");
  if(!playerAction||!enemyAction) throw new Error("双方の行動がそろっていません。");

  // オンライン対戦ではCPU用runTurnを使わず、2人分の確定した行動をそのまま処理する。
  // これにより、相手が実際に選んだmoveIdをCPU用のランダム技へ置き換えてしまう問題を防ぐ。
  const makeAction=(side,other,act)=>{
    if(act.type==="switch") return {type:"switch",index:Number(act.index),priority:6};
    if(act.type!=="move") throw new Error("不正なオンライン行動です。");
    const m=getMove(String(act.moveId||act.id||""));
    if(!m) throw new Error("選択された技が見つかりません。");
    const i=side.active,p=side.team[i];
    if(!Array.isArray(p.moves)||!p.moves.map(String).includes(String(m.id))) throw new Error("その技は使用できません。");
    return {type:"move",move:m,priority:priorityWithAbility(b,side,m,other)};
  };

  const pa=makeAction(b.player,b.enemy,playerAction);
  const ea=makeAction(b.enemy,b.player,enemyAction);
  prepareTurn(b);
  const actions=[{side:b.player,other:b.enemy,act:pa},{side:b.enemy,other:b.player,act:ea}];
  actions.sort((x,y)=>{
    if(x.act.priority!==y.act.priority) return y.act.priority-x.act.priority;
    const sx=effectiveSpeed(x.side,x.side.team[x.side.active]);
    const sy=effectiveSpeed(y.side,y.side.team[y.side.active]);
  ...
  for(const x of actions){
    if(b.over) break;
    const side=x.side,other=x.other,act=x.act;
    if(side.hp[side.active]<=0) continue;
    if(act.type==="switch"){
      const n=act.index;
      if(!Number.isInteger(n)||!side.selected.includes(n)||side.hp[n]<=0||n===side.active) throw new Error("そのポケモンには交代できません。");
      const old=side.team[side.active].name;
      side.active=n;
      b.log.push(`${side===b.player?"自分":"相手"}は${side.team[n].name}に交代した！`);
      applySwitchIn(b,side);
      continue;
    }
    if(side.volatile[side.active]?.flinch){b.log.push(`${side.team[side.active].name}はひるんで動けない！`);continue;}
    if(!canAct(b,side,side.active)) continue;
    const result=doMove(b,side,other,act.move)||{switched:false};
    resolveFaints(b);
    if(b.phase==="switch_required") break;
    if(result.switched && !b.over){
      if(side===b.player){b.phase="switch_required";break;}
      const alive=side.selected.filter(i=>side.hp[i]>0&&i!==side.active);
      if(alive.length){const n=alive.sort((a,c)=>side.hp[c]-side.hp[a])[0];side.active=n;b.log.push(`相手は${side.team[n].name}を繰り出した！`);applySwitchIn(b,side);}
    }
  }

  if(!b.over&&b.phase==="battle"){
    endTurnSide(b,b.player);
    endTurnSide(b,b.enemy);
    resolveFaints(b);
  }
  if(!b.over&&b.phase==="battle"){
    b.player.megaThisTurn=false; b.enemy.megaThisTurn=false;

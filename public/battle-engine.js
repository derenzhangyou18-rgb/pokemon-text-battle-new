...
function doMove(b,attSide,defSide,move){
  ...
  if(defSide.hp[di]>0 && move.contact){
    ...
    if(revengeItem?.effect==="contact_damage_1_6_item"){
      const rd=Math.max(1,Math.floor(defSide.maxHp[di]/6));
      attSide.hp[ai]=Math.max(0,attSide.hp[ai]-rd);
      b.log.push(`${a.name}は${d.name}の持ち物「${revengeItem.name||itemId(d)}」で最大HPの1/6のダメージを受けた！`);
    }
  }
...
function runOnlineForcedSwitchMove(b,sideName,action){
  ...
  if(!b.over && result.switched && alive.length){
    ...
    // 交代先選択中は「技を送信済み」の状態を残さず、交代入力だけを受け付ける。
    b.pendingPlayerMoveId=null;
    b.pendingEnemyMoveId=null;
    b.pendingEnemyAction=null;
...
function runOnlineTurn(b,playerAction,enemyAction){
  normalizeBattleState(b);
  if(b.over) throw new Error("この対戦は終了しています。");
  if(b.phase!=="battle") throw new Error("現在はオンラインの行動を処理できません。");
  const actions=[{side:b.player,other:b.enemy,act:playerAction},{side:b.enemy,other:b.player,act:enemyAction}];

  // オンラインでも通常対戦と同じ「優先度 → 素早さ → 同速ランダム」の順で処理する。
  // 先に双方の技を pending に入れておくことで、相手の技種別を条件にする効果も正しく判定できる。
  b.pendingPlayerMoveId=playerAction?.type==="move" ? String(playerAction.moveId||playerAction.id||"") : null;
  b.pendingEnemyMoveId=enemyAction?.type==="move" ? String(enemyAction.moveId||enemyAction.id||"") : null;
  b.pendingEnemyAction=enemyAction||null;

  const actionPriority=(x)=>{
    if(x.act?.type==="switch") return 6;
    const m=getMove(x.act?.moveId||x.act?.id);
    return m ? priorityWithAbility(b,x.side,m,x.other) : -999;
  };
  actions.sort((x,y)=>{
    const px=actionPriority(x), py=actionPriority(y);
  ...
  for(const x of actions){
    ...
    if(act.type==="switch"){
      const n=Number(act.index);
      if(side.selected.includes(n)&&side.hp[n]>0&&n!==side.active){
        side.active=n;
    ...
    const m=getMove(act.moveId||act.id);
    if(m) doMove(b,side,other,m);
    resolveFaints(b);
  }
  if(!b.over&&b.phase==="battle"){

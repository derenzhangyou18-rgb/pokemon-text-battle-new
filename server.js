...
// Netlify Functions run from /var/task, which is read-only. Never create the local
// data directory during module initialization; local mode creates it only when writing.
...
function saveData(){
  ...
  saveQueue=saveQueue.then(async()=>{
    if(isNetlifyRuntime()) await blobStore().set("database.json", payload, { metadata:{updatedAt:new Date().toISOString()} });
      ...
      fs.writeFileSync(LOCAL_DATA_PATH,payload,"utf8");
    }
  });
  return saveQueue;
}
...
const loginLimiter = rateLimit({
  ...
  legacyHeaders: false,
  // Netlify + serverless-http does not always populate Express's req.ip.
  // Provide a safe fallback so express-rate-limit can build its partition key.
  keyGenerator: (req) => {
    ...
    return req.ip || first || "netlify-anonymous";
  },
